...
def create_vm(parameters, vdi_group):
    ...
    if parameters['newContainer']['storage'] not in storage_repo_names:
        logger.error(
            f"[{vdi_group}] Configured storage {parameters['newContainer']['storage']} is not available on {parameters['masterNode']}")
        raise ValueError(f"[{vdi_group}] Configured storage {parameters['newContainer']['storage']} is not available on {parameters['masterNode']}")
...
def create_master(group_data, vdi_group):
    ...
    try:
        ...
        logger.error("Could not create vm")
        return False
