        ...
        if values[1] == masterHostname:
            if (values[3] == masterMAC):
                if (values[4] == masterIp):
                    logger.info(
                        "*** Master Configuration is consistent to devices.csv ***"
                    )
                    return True
                else:
                    logger.info(
                        "*** PROBLEM: IP doesnt match with Configuration ***")
            else:
                logger.info(
                    "*** PROBLEM: MAC doesnt match with Configuration ***")
    else:
...
def setLinboRemoteCommand(schoolId, masterHostname):
    try:
        if multischool == True:  # linbo-remote for one multischool school
            command2 = "linbo-remote -s " + str(
                schoolId
            ) + " -i " + masterHostname + " -p partition,format,initcache:rsync,sync:1,start:1"
            setCommand(command2)
            print(command2)
...
def createVM(parameters):
    ...
    logger.info("*** Master-VM with " +
                str(parameters["newContainer"]["vmid"]) +
                " is being created... ***")
...
def deleteFailedMaster(masterVmid):
    try:
        ...
        if status == "running":
            ...
            logger.info("*** Failed Master " + str(masterVmid) +
                        " is getting stopped.***")
...
def main(vdiGroup):
    logger.info("*** Creating new Master begins for Group " + vdiGroup +
                " begins ***")
    ...
    if schoolId != "" or schoolId != "default-school":
        devicePath = "/etc/linuxmuster/sophomorix/" + str(
            schoolId) + "/" + str(schoolId) + ".devices.csv"
    else:
        devicePath = "/etc/linuxmuster/sophomorix/default-school/devices.csv"

    #masterName = str(schoolId) + "-" + vdiGroupInfos['name']
    ...
    masterSata0 = masterStorage + ":" + str(
        masterSize) + ",format=" + masterFormat
    ...
    if "tag" in vdiGroupInfos:
        ...
        if masterTag != 0:
            masterNet0 = "bridge=" + masterBridge + ",virtio=" + masterMac + ",tag=" + str(
                masterTag)
        else:
    ...
    setLinboRemoteCommand(schoolId,
                          masterHostname)  # and sets linbo-remote command
    parameters = {
    ...
    ports = nmapPorts
    if checkNmap(masterNode, timeout, masterVmid, masterIp,
                 ports) == True:  # check if windows bootet succesfully
        masterDescription['buildstate'] = "finished"
        description = json.dumps(masterDescription)
        prepareTemplate(masterNode,
                        masterVmid)  # convert VM to Template if stopped
        proxmox.nodes(masterNode).qemu(masterVmid).config.post(
            description=description)
        logger.info("*** Creating new Template for group " + vdiGroup +
