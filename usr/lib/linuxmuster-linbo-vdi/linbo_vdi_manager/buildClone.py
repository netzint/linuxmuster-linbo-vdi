...
def get_next_available_vmid(clone_states, id_range, vdi_group):
    ...
    for id in id_range:
        if id not in clone_states:
            ...
            return id
    logger.warning(
        f"[{vdi_group} No Free VMIds left for cloning , add VMs in devices.csv on server!")
    return False
...
def generate_clone_description(group_data, masterVmid, cloneName, vdi_group):
    ...
    dateOfCreation = timestamp.strftime(
        "%Y%m%d%H%M%S")         # => "20201102141556"
...
def clone_master(master_node, master_vmid, clone_vmid, clone_name, clone_description, vdi_group):
    # important! sonst liest nur die Haelfte
    description = json.dumps(clone_description)
    logger.info(f"[{vdi_group} Clone-VM-Name: {clone_name}")
    proxmox.nodes(master_node).qemu(master_vmid).clone.post(
        newid=clone_vmid, name=clone_name, description=description)
...
def start_clone(clone_node, clone_vmid, vdi_group):
    ...
    try:
        ...
        logger.info(
            f"[{vdi_group} VM {clone_vmid} couldn't get started, script is terminating")
        return
    ...
    while time.time() < terminate:
        ...
        for port in linux_ports:
            try:
                ...
                continue
        logger.info("*** Clone " + str(clone_vmid) +
...
def waitForStatusRunning(timeout, clone_node, clone_vmid):
    terminate = time.time() + timeout
    while time.time() < terminate:
        status = proxmox.nodes(clone_node).qemu(
            clone_vmid).status.current.get()
...
def build_clone(clone_states, group_data, master_vmid, vdi_group):
    ...
    clone_description = generate_clone_description(
    ...
    cloneNet = "bridge=" + group_data['bridge'] + \
        ",virtio=" + clone_network_info['mac']
